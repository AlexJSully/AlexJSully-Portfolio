...
- **Mock external I/O and platform APIs only**: `@configs/firebase`, `next/navigation`, timers, `navigator`. Never mock internal helpers, utilities, or domain logic. Read mock state with `jest.requireMock(...)` or `as jest.MockedFunction<typeof fn>`, never `require()`.
...
- **Test setup**: `jest/setup.ts` with the jsdom environment.
- **E2E**: specs in `cypress/e2e/`, every `describe` closing with `afterEach(() => { cy.a11yCheck(); })` (cypress-axe). No `baseUrl` is set, so specs call `cy.visit('http://localhost:3000')`.
