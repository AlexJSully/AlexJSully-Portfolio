...
- **Promises fail silently**, so each of these is a defect: a promise neither awaited nor returned, an `async` callback handed to a non-awaiting iterator such as `forEach`, and `map` producing promises with no `Promise.all` around them
