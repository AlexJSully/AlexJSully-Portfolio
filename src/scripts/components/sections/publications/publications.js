            ...
            if (pubs[i]?.['related-project']) {
                if (ProjectsData[pubs[i]['related-project']]) {
                    relatedProjectData.push(
                        <span className="publications-URL" key={`research-related-${ProjectsData[pubs[i]['related-project']].name}-url`}>
                        ...
                        <Card className="publications-Card" key={`research-card-${pubs[i]?.title}`}>
                            <CardActionArea key={`research-card-${pubs[i]?.title}-actionArea`}>
                                <CardContent key={`research-card-${pubs[i]?.title}-content`}>
                                    <Typography variant="h5" component="h2"  key={`research-card-${pubs[i]?.title}-title`}>
                                        {pubs[i]?.title}
                                    </Typography>
                                    <Typography className="publications-meta publications-authors" variant="body3" component="p"  key={`research-card-${pubs[i]?.title}-authors`}>
                                        {pubs[i]?.authors.join(', ')}
                                    </Typography>
                                    <Typography className="publications-meta" variant="body3" component="p"  key={`research-card-${pubs[i]?.title}-metaData`}>
                                        {metaData?.length > 0 ? metaData.join(' | ') : null}
                                    </Typography>
                                    <Typography className="publications-meta" variant="body3" component="p" key={`research-card-${pubs[i]?.title}-relatedProjects`}>
                                    ...
                                    <Typography className="publications-abstract" variant="body1" component="p" key={`research-card-${pubs[i]?.title}-abstract`}>
                                        {abstract}
                                    </Typography>
                                </CardContent>
                            </CardActionArea>
